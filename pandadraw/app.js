...
px.$pixluminosity = document.getElementById('luminosity');
...
function importState(jd) {
    px.$pixluminosity.value = (jd.luminosity * 255) ;
    cols = jd.width ;
    rows = jd.height ;
    for (j = 0 ; j < cols ; j++) {
                        for (i=0 ; i<rows ; i++) {
                            id = i+j*cols ;
                            str = "rgb("+jd.pixels[(i*cols+j)*3]+","+jd.pixels[(i*cols+j)*3+1]+","+jd.pixels[(i*cols+j)*3+2]+")" ;
                            px.$pixels[i+j*cols].style.backgroundColor = str ;
                        }
                    }
}
...
function pushState() {
	rows = px.$paper.rows.length;
	cols = px.$paper.rows[0].cells.length;
	var pixx = [];
    for (j = 0 ; j < cols ; j++) {
        for (i=0 ; i<rows ; i++) {
            str = px.$pixels[j+i*rows].style.backgroundColor ;
            // console.log(str+' '+mtpickColor(str)+' '+typeof(mtpickColor(str)) );
            arr = mtpickColor(str) ;

            pixx = pixx.concat(arr) ;
        }
    }

    console.log(pixx.length) ;
    mtsendAjaxRequest(pixx, 0) ;
}
...
px.$paper.addEventListener('touchend', drawEnd, false);
function drawStart(e) {
	e.preventDefault();
	e.target.style.backgroundColor = px.selected;
	console.log(e.target.id) ;

//	mtsendAjaxRequest(mtpickColor(px.selected), e.target.id) ;
}
function drawMove(e) {
...
// luminosity
px.$pixluminosity.onchange = function (e) {
    console.log(px.$pixluminosity.value)
    mtsendLum(px.$pixluminosity.value) ;
    pushState() ;
};
